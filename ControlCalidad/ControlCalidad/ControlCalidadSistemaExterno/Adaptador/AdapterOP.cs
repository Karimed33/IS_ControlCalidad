...
namespace ControlCalidadSistemaExterno.Adaptador
{
    public class AdapterOP
    {
        ...
        {
            var api = "https://localhost:44315/api/OrdenProduccion_VerOP{codOp}?codOP=" + codOP;
            var resultado = conectar.Gets(api);
            OrdenProduccion op = new OrdenProduccion();
            op = JsonConvert.DeserializeObject<OrdenProduccion>(resultado);
            return op;
        ...
        {
            var api = "https://localhost:44315/api/OrdenProduccion_VerOP{codOp}?dni=" + dni;
            var resultado = conectar.Gets(api);
            OrdenProduccion op = new OrdenProduccion();
            op = JsonConvert.DeserializeObject<OrdenProduccion>(resultado);
            return op;
